...
# # Phase 3 – config-based doctor routing (temporary)
# from doctor_config import DOCTORS

# # Phase 6 – DB onboarding
# from db.repository import create_doctor, doctor_exists, get_doctor_by_slug,get_doctor_by_email, get_upcoming_appointments_for_doctor,get_appointment_by_id, cancel_appointment_db , reschedule_appointment_db,get_doctor_calendar_credentials
...
# # -------------------------------
# # Doctor resolution helper (config-based for now)
# # -------------------------------
...
#     # 🔒 ALWAYS create a new session if missing
#     if not session_id:
#         session_id = str(uuid.uuid4())

#     # 🔒 Ensure isolated state per session
#     if session_id not in state_store:
#         state_store[session_id] = BookingState()

#     state = state_store[session_id]

#     # 🔒 HARD RESET — NON-NEGOTIABLE
#     state.reset_flow()
#     state.doctor_id = doctor["id"]          # MUST be string UUID
...
#     # 🔒 Set cookie so /chat uses SAME session
#     response.set_cookie(
#         key="session_id",
#         value=session_id,
#         httponly=True
...
#     """
#     Phase 7.2.2
#     Start OAuth flow for a specific doctor.
#     """
#     doctor = get_doctor_by_slug(doctor_slug)
#     if not doctor:
#         raise HTTPException(status_code=404, detail="Doctor not found")
...
#     # Transitional: keep in-memory credentials for now
...
#         name=payload.name,
#         email=payload.email,
#         slug=slug,  # 👈 REQUIRED (next change)
...
#     """
#     Phase 7.3.1
#     Read-only appointment listing for doctor.
#     """
...
#     """
#     Phase 7.3.2
#     Doctor-side appointment cancellation.
#     """
...
#     """
#     Phase 7.3.3
#     Doctor-side appointment reschedule.
#     """
...
#     # Availability check (reuse existing logic)
...
#     # DB-first reschedule (same appointment row)
...
#     # TODO (Phase 8): notify patient about reschedule
